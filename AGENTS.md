...
- Three components, each self-contained with no shared build tooling: `extension/` (Chrome MV3 content script, no build step - load unpacked), `relay-server/` (plain Node `http`, no deps), `vault-tool/` (Node CLI, no deps). See `README.md` for setup/usage of all three.
